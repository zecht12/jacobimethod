...
const JacobiMethod: React.FC = () => {
    ...
    return (
        <Autof style={styles.container}>
            ...
            {iterations.length > 0 && (
                <DataTable style={styles.resultTable}>
                    <DataTable.Header>
                        <DataTable.Title style={styles.resultText}>Iterasi</DataTable.Title>
                        {Array.from({ length: iterations[0].length }, (_, i) => (
                            <DataTable.Title key={i} style={styles.resultText}>x{i + 1}</DataTable.Title>
                        ))}
                    </DataTable.Header>
                    {iterations.map((iter, index) => (
                        <DataTable.Row key={index}>
                            <DataTable.Cell style={styles.resultText}>{index + 1}</DataTable.Cell>
                            {iter.map((val, i) => (
                                <DataTable.Cell key={i} style={styles.resultText}>{val.toFixed(6)}</DataTable.Cell>
                            ))}
                        </DataTable.Row>
                    ))}
                </DataTable>
